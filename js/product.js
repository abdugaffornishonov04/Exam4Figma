...
function GetPrCards(products){
  const prCard = document.createElement("div");

  const prCardBody = document.createElement("div");
  const prCardBodyImg = document.createElement("img");
  prCardBodyImg.src = `${products[0].images}`;
  prCardBodyImg.alt = `${products[0].name}`;
  prCardBodyImg.width = "100%";
  prCardBody.appendChild(prCardBodyImg);

  const prCardMain = document.createElement("div");
  const prCardMainText = document.createElement( "p" );
  prCardMainText.innerHTML = `${products[0].description}`;
  const prCardMainTitle = document.createElement( "p" );
  prCardMainTitle.innerHTML = `${products[0].category}`;
  const prCardMainRating = document.createElement( "p" );
  prCardMainRating.innerHTML = `${products[0].rating}`;
  prCardMain.append( prCardMainText, prCardMainTitle, prCardMainRating );

  prCard.append(prCardBody, prCardMain);

  return prCard;

};

const card = GetPrCards( products );
